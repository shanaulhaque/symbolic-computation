...
public class ParseCombination extends Parser implements Runnable  
{
	String in="",out="";
	Thread t;
	int cnt=0;
	
	ParseCombination(String in1,int cnt)
	{
		this.cnt=cnt;
		in=in1;
		t=new Thread(this);
	...
	private static double evaluate(double n,double r)
	{
		...
		double d2=ParseFactorial.parse(r);
		double d3=ParseFactorial.parse(n-r);
		return d1/(d2*d3);
	}
}