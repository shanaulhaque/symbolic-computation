...
public class Parser 
{
	String input;
	String str;
	protected String [] TempOut=new String[50];
	...
	public String[] prepareParse()
	{
		...
		{
			String []finalString={};
			finalString[0]="Input contian Space or enter";
			return finalString;
		}
		else
		{
	...
	public String parse()throws Exception
	{
		...
		for(int i=0;i<in.length;i++)
		{
			...
			{
				ParsePermutation x=new ParsePermutation(in[i],1);
				TempOut[tempIndex]=x.out;
				tempIndex++;
			}
			else if(in[i].startsWith("combi"))
			{
				ParseCombination x=new ParseCombination(in[i],1);
				TempOut[tempIndex]=x.out;
				tempIndex++;
			}
			else if(in[i].startsWith("sin"))
			{
				...
				ParseSin x=new ParseSin(in[i],1);
				TempOut[tempIndex]=x.out;
				tempIndex++;
			...
			{
				ParseCosec x=new ParseCosec(in[i],1);
				TempOut[tempIndex]=x.out;
				tempIndex++;
			...
			{
				ParseTan ob=new ParseTan(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("sec"))
			{
				ParseSec ob=new ParseSec(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("cot"))
			{
				ParseCot ob=new ParseCot(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("log1"))
			{
				ParseLog ob=new ParseLog(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("logb"))
			{
				ParseLogb ob=new ParseLogb(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("ln"))
			{
				ParseLn ob=new ParseLn(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("hsin"))
			{
				ParseSinh ob=new ParseSinh(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("htan"))
			{
				...
				ParseTanh ob=new ParseTanh(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("hcosec"))
			{
				ParseCosech ob=new ParseCosech(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("hcos"))
			{
				...
				ParseCosh ob=new ParseCosh(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("hsec"))
			{
				...
				ParseSech ob=new ParseSech(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("hcot"))
			{
				ParseCoth ob=new ParseCoth(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			...
			{
				ParseSini ob=new ParseSini(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("icosec"))
			{
				ParseCoseci ob=new ParseCoseci(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("icos"))
			{
				...
				ParseCosi ob=new ParseCosi(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("itan"))
			{
				ParseTani ob=new ParseTani(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("isec"))
			{
				ParseSeci ob=new ParseSeci(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("icot"))
			{
				ParseCoti ob=new ParseCoti(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("deriv"))
			{
				ParseDifferentiation ob=new ParseDifferentiation(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].startsWith("prod"))
			{
				ParseProduct ob=new ParseProduct(in[i],1);
				TempOut[tempIndex]=ob.out;
				tempIndex++;
			}
			else if(in[i].contains("!"))
			{
				  ParseFact ob=new ParseFact(in[i],1);
				  TempOut[tempIndex]=ob.out;
				  tempIndex++;
			}
			else if(in[i].startsWith("indefint"))
			{
				ParseIndefInt ob=new ParseIndefInt(in[i],1);
				String [] temp=ob.out;
				for(int j=0;j<temp.length;j++)
				{
					TempOut[tempIndex]=temp[j];
			...
			{
				ParseDefiniteIntegral ob=new ParseDefiniteIntegral(in[i],1);
				String [] temp=ob.out;
				for(int j=0;j<temp.length;j++)
				{
					TempOut[tempIndex]=temp[j];
